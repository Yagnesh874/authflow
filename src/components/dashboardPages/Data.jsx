...
const Data = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const citySet = new Set(data.users.map((user) => user.company.address.city));
        const departmentSet = new Set(data.users.map((user) => user.company.department));
  ...
  const getFilteredUsers = () => {
    return data?.filter((user) => {
      const cityMatch = selectCity === "" || user.company?.address?.city === selectCity;
      const departmentMatch = deptName === "" || user.company?.department === deptName;
      const ageMatch = () => {
        if (age === "") return true;
      ...
      const nameMatch = userName === "" || `${user.firstName} ${user.lastName}`.toLowerCase().includes(userName.toLowerCase());
      return cityMatch && departmentMatch && ageMatch() && nameMatch;
    }) || [];
  };

  const filteredUsers = getFilteredUsers().slice((currentPage - 1) * usersPerPage, currentPage * usersPerPage);
      ...
      <div className="main-content-2">
        <div className="content">
          <div className="page-header">
            ...
            <p className="page-subtitle">Welcome back! Here's what's happening with your users.</p>
          </div>
          ...
          <div className="table-container">
            <div className="table-header">
              ...
              <div className="filters">
                ...
                <select className="filter-select" value={selectCity} onChange={(e) => setSelectedCity(e.target.value)}>
                  <option value="">All Cities</option>
                  {[...city].map((c) => <option key={c} value={c}>{c}</option>)}
                </select>

                <select className="filter-select" value={deptName} onChange={(e) => setDeptName(e.target.value)}>
                  <option value="">All Department</option>
                  {[...department].map((d) => <option key={d} value={d}>{d}</option>)}
                </select>
                ...
                <div className="search-container">
                  <input type="text" className="search-input" placeholder="Search By Name" value={userName} onChange={(e) => setUserName(e.target.value)} />
                </div>
            ...
            {getFilteredUsers().length > usersPerPage && (
              <div className="pagination">
                {Array.from({ length: Math.ceil(getFilteredUsers().length / usersPerPage) }).map((_, index) => (
                  <button
                    key={index}
